<?php
    ...
    //poke_move table
    $query = "SELECT type_name, type_weakness 
                FROM typedisad
                INNER JOIN typelist
                    USING (type_id);";
    ...
    $del_stmt = $conn->prepare("DELETE FROM typead WHERE type_id = ? AND type_weakness = ?;"); 
    ...
    function result_to_deletable_table($result){ 
        ...
        <form action="deleteTypeDisad.php" method=POST>
        <table>
            <thead>
            <tr>
                <th> Delete batch? </th>
